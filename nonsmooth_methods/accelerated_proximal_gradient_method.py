...
def acclerated_proximal_grad_descent(beta_0, weights, X, Y, step_size, max_iter, lam, type):
    beta = beta_0
    include_int = False
    list_betas = [np.zeros(beta.shape), np.zeros(beta.shape)]
    ...
    for iterations in range(2, max_iter):
        v = beta + (iterations - 2) / (iterations + 1) * (beta - list_betas[iterations - 1])
        sgd_step = v - step_size * mse_grad(X_tilde, beta, Y_tilde)
        beta = prox_operator_glasso(sgd_step, lam, weights, step_size, type)
        f_train = mse(X_tilde, beta, Y_tilde) + group_lasso_regularizer(beta, weights)
        f_test = mse(X_test, beta, y_test) + group_lasso_regularizer(beta, weights)
    ...
    return beta, np.array(fs_train), np.array(fs_test)
...
if __name__ == '__main__':
    ...
    beta, f_train, f_test = acclerated_proximal_grad_descent(beta_0=beta_0,
    ...
    fig = plt.figure()
    plt.title(r"Accelerated LASSO $f^k-f^*$")
    ax = fig.add_subplot(1, 1, 1)
    p1 = ax.plot(f_train - f_star)
    p2 = ax.plot(f_test - f_star)
    handles = [p1[0], p2[0]]
